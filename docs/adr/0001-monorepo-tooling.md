...
## Decision
Use Nx with pnpm workspaces for task orchestration. The root will contain shared tooling configs in `configs/` and workspace scripts in `package.json`.

## Consequences
- Fast, cacheable builds and tests across apps.
- Strong workspace enforcement and dependency graph tooling.
- Requires contributors to use pnpm and Nx.
